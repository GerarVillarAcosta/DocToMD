...
def test_markitdown_convert_returns_string():
    ...
    with patch("doctomd.converters.markitdown.MarkItDown") as MockMD:
        MockMD.return_value.convert.return_value = mock_result
        from doctomd.converters.markitdown import convert
...
def test_docling_convert_returns_string():
    ...
    with patch("doctomd.converters.docling.DocumentConverter") as MockDC:
        MockDC.return_value.convert.return_value = mock_result
        from doctomd.converters.docling import convert
        result = convert(Path("test.pdf"))
